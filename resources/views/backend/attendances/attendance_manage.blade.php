...
@section('title', 'Attendance Management | ' . Config::get('adminlte.title'))
...
<script>
    ...
    $(document).ready(function () {
        $('#editModal').on('show.bs.modal', function (event) {
            ...
            var workerId = button.data('worker_id');
            var workerRoleId = button.data('worker_role_id');
            var attStatus = button.data('status');
            var checkIn = button.data('check_in');
            var checkOut = button.data('check_out');
            try {
            // Use JSON.parse instead of $.parseJSON
            var attendanceStatus = button.data('attendance_status');
 
            var modal = $(this);
            var dropdown = modal.find('#attendanceStatusDropdown');

            if (dropdown.length) {
                // Clear existing options
                dropdown.empty();

                // Iterate through the options array and add them to the dropdown
                for (var i = 0; i < attendanceStatus.length; i++) {
                    var option = $('<option></option>');
                    option.val(attendanceStatus[i].id);
                    option.text(attendanceStatus[i].name);
                    dropdown.append(option);
                }

                // Set the selected option based on the data
                dropdown.val(attStatus);
            }

            modal.find('#attendanceId').val(dataId);
            modal.find('#attendanceDate').val(attendanceDate);
            modal.find('#workerId').val(workerId);
            modal.find('#workerRoleId').val(workerRoleId);
            modal.find('#checkIn').val(checkIn);
            modal.find('#checkOut').val(checkOut);
        } catch (error) {
