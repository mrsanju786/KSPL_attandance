...
@section('title', 'Leave | ' . Config::get('adminlte.title'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title"> Staff Leave List</h3>
            <br>
            

            <form class="form-inline" action="{{ url('leave')}}" method="GET" style="float: right;">
                ...
                <div class="form-group mx-sm-3 mb-2">
                    ...
                    @foreach ($roles as $value )
                        <option value="{{$value->id}}" {{$name == $value->id  ? 'selected' : ''}}>{{$value->display_name}}</option>
                    @endforeach
                </select>
                </div>
