...
class TSMReportExport implements FromView
{
    /**
    ...
    public function view(): View
    {
        ...
        $absentUser =[];
        // $all_users = User::where('role', 5)
        //                 ->get('id');
                       
        $activeUsers = User::where('role', 5)
        ->where('status',1)
        ->get();

        $deactiveusers = User::where('role', 5)
                ->where('status',0)
