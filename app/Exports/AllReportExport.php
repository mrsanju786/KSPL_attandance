...
class AllReportExport implements FromView
{
    ...
    public function view(): View
    {
        ...
        $activeUsers = User::whereIn('role', [3,5,6])
                ->where('status',1)
                ->get();
        
        $deactiveusers = User::whereIn('role', [3,5,6])
                ->where('status',0)
        ...
        foreach($absentUser as $key=>$values){
            ...
            foreach ($dates as $date) {
                ...
                if ($attendance == null) {
                    ...
                    if (!empty($attendance->status)) {
                        if ($attendance->status == '1') {
                            ...
                            $el = $el+1;
                            $date_attendance = "EL";
                        } elseif ($attendance->status == '12') {
                            $cl = $cl+1;
                            $date_attendance = "CL";
                        } elseif ($attendance->status == '13') {
                            $sl = $sl+1;
                            $date_attendance = "SL";
                        }

                        if($attendance->status=='1' || $attendance->status=='7' || $attendance->status=='3' || $attendance->status=='8' || $attendance->status=='11' || $attendance->status=='12' || $attendance->status=='13'){
                            $p_count++;
                        }

                        if($attendance->status=='8'  || $attendance->status=='12' || $attendance->status=='13'){
                            $l_count++;
                        }
                ...
                $values->in_time = !empty($attendance->in_time) ? $attendance->in_time  : "-";
                $values->out_time = !empty($attendance->out_time) ? $attendance->out_time  : "-";
            ...
            $values->early_leave = !empty($el) ? $el: 0;
            $values->out_door = !empty($od) ? $od: 0;
            $values->week_off = !empty($wo) ? $wo: 0;
            $values->leave = !empty($l_count) ? $l_count: 0;
            $values->holiday = !empty($h) ? $h: 0; 
            $values->absent = !empty($a) ? $a: 0;

        }

        return view('backend.exports.all_attendance', ['absent_users'=>$absentUser, 'dates'=>$dates
        ]);
    }
}
