    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($currentData) {
                ...
                //     $new['id_card_signature'] = null; // filename for db

                //     if ($currentData->{'id_card_signature'} != 'default-user.png') {
                //         @unlink(Config::get('const.UPLOAD_PATH') . $currentData['id_card_signature']);
                //     }
                // }
                ...
                // upload image
                    ...
                    if ($request->hasFile('image')) {
                        ...
                        ${'image'} = time().$currentData->id . "id_card_signature." . $file->getClientOriginalExtension();
                        $new['id_card_signature'] = ${'image'};
                        // save image to the path
                        $file->move(Config::get('const.UPLOAD_PATH'), ${'image'});
                    } else {
                        $new['id_card_signature'] = 'default-user.png';
                    }
            

                $currentData->id_card_signature=$new['id_card_signature'];
                $currentData->save();
                // Update
